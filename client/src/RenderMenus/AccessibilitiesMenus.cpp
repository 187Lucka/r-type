...
void Menu::applyDaltonianColors() {
    if (isDaltonianMode) {
        ...
        // Boutons des rooms
        for (auto& button : roomButtons) {
            button.setFillColor(roomColor);
        }
        
        // Boutons "Rejoindre"
        for (auto& button : joinButtons) {
            button.setFillColor(joinColor);
        }
...
void Menu::updateAccessibilityStyles() {
    if (isHighContrastMode) {
        // Ajout de la mise à jour du texte des paramètres
        settingsText.setFont(robotoFont);
        settingsText.setFillColor(sf::Color::Black);
        settingsText.setCharacterSize(settingsText.getCharacterSize() * 1.2f);
    ...
    chatInputText.setFillColor(isHighContrastMode ? sf::Color::Black : sf::Color::White);
       ...
       // registerText.setFillColor(sf::Color::Black);
        ...
        sf::Color textColor = isDaltonianMode ? sf::Color::Yellow : sf::Color::White;
